...
function auSuivant() {
  ...
  ////console.log("auSuivant 1");
    ...
    //parent.boutons.document.getElementById('Brejouer').style.visibility='hidden';
      ...
      if (parent.ba.serie == 8)parent.og.location = "resumeFrame" + parent.ba.program+ parent.ba.activity + ".html?version=43";
...
function montreTxt1(m) {
  ...
  for (var i=0; i<t.length - 1; i++) {
    ...
    newTxt += "<div><span style='line-height:16px;'>"  + tbl + "</span></div>";
...
function addSuffix(m,n) {
  ...
  for (var i=0; i<t.length - 1; i++) {
    ...
    var w=frames[m].txtSize(pref);
    var IdP = "Pref"+(i+1);
    var IdS = "Suff"+(i+1);
    if (i <= n){
    
    tbl = "<table Id='" + IdP + "' style='position:absolute;top:"+topPos+"px;left:1px;vertical-align:middle'><tr><td >" + pref + "</td></tr></table>\
           <table Id='" + IdS + "' style='position:absolute;top:"+topPos+"px;left:"+(w+2)+"px;vertical-align:middle'><tr><td class='suffix' style='width:22px;border: 1px solid #000;background:#00ff00;text-align:center'>" + tSuf +"</td></tr></table>";
    ////console.log(tbl);
    } else {
...
function collapse(m,n) {
  ...
  for (var i=0; i<t.length - 1 - n; i++) {
    ...
    col2Txt += "<table Id='" + Id + "' style='position:absolute;top:"+topPos+"px;right:5px;vertical-align:middle'><tr><td  class='suffix' style='width:22px;border: 1px solid #000;background:#00ff00;text-align:center'>" + tSuf +"</td></tr></table>";
...
function montreAbstractions0(m,n) {
  ...
  var newTxt = "<div class='abstractions' style='visibility:hidden'><span style='float:right;width:34px;line-height:14px;'>";
  for (var i=1; i <= n; i++) {
   //alert("i="+i + " " + t[i]);
   suf = spSuffix[i][m];
   if (suf == "") {suf="&nbsp"}
