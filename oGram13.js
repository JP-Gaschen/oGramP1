...
function init() {
  ...
  //obj = parent.ba.document.getElementById("enCours");
  //cligne(obj,4,"alert('init');");
  if (parent.gPhase == 1) {
    gPhase = 1
...
function continuer() {
  ...
  setTimeout(function(){frames['Resume2'].window.scrollTo(0,3000)},100); //window.scrollTo(0,3000); //
}
...
function auSuivant() {
  ...
  ////console.log("auSuivant 1");
    ...
    if (parent.ba.serie == 6) parent.og.location = "resumeFrame" + parent.ba.program+ parent.ba.activity + ".html?version=43";
      ...
      if (parent.isDemo) {
        ...
        gIgnoreClick = true;
        parent.boutons.document.getElementById('diffuse').innerHTML = "observez";
        parent.boutons.document.getElementById('Bvalider').style.visibility='hidden';
        parent.boutons.document.getElementById('Brejouer').style.visibility='hidden';
...
function addSuffix(m,n) {
  ...
  for (var i=0; i<t.length - 1; i++) {
    ...
    var w=frames[m+7].txtSize(pref);
    var IdP = "Pref"+(i+1);
    var IdS = "Suff"+(i+1);
    if (i <= n){
    
    tbl = "<table Id='" + IdP + "' style='position:absolute;top:"+topPos+"px;left:1px;vertical-align:middle'><tr><td >" + pref + "</td></tr></table>\
           <table Id='" + IdS + "' style='position:absolute;top:"+topPos+"px;left:"+(w+2)+"px;vertical-align:middle'><tr><td class='suffix' style='width:22px;border: 1px solid #000;background:#cc60ff;text-align:center'>" + tSuf +"</td></tr></table>";
    ////console.log(tbl);
    } else {
      tbl = "<table  style='position:absolute;top:"+topPos+"px;vertical-align:middle'><tr><td>" + t[i] + "</td></tr></table>";
    }
    
    newTxt += "<span style='line-height:14px;'>"  + tbl + "</span>";
...
function collapse(m,n) {
  ...
  for (var i=0; i<t.length - 1 - n; i++) {
    ...
    col2Txt += "<table Id='" + Id + "' style='position:absolute;top:"+topPos+"px;right:5px;vertical-align:middle'><tr><td  class='suffix' style='width:22px;border: 1px solid #000;background:#cc60ff;text-align:center'>" + tSuf +"</td></tr></table>";
  ...
  newTxt += "<span style='width:30px;line-height:14px;'>" + col2Txt + "</span>";
  newTxt += "</div>";
  ////console.log(newTxt);
...
function montreAbstractions01(m) {
  ...
  var serie = parent.ba.serie;
  var newTxt = "<div><span style='float:right;width:34px;line-height:14px;'>";
  ...
  newTxt += "</span></div><div style='clear:both'></div>";
  ////console.log(newTxt);
...
function montreAbstractions03(m,n) {
  ...
  //if (m == 0 && n == 1) clear_all();
  var serie = parent.ba.serie;
  var newTxt = "<div class='abstractions' style='visibility:hidden'><span style='float:right;width:30px;line-height:14px;'>";
  for (var i=1; i <= n; i++) {
   //alert("i="+i + " " + t[i]);
   suf = spmfSuffix03[i][m];
   if (suf == "") {suf="&nbsp"}
   
    tbl = "<table style='vertical-align:middle;'><tr><td id='" + m + "'' class='suffix' style='width:22px;border: 1px solid #000;text-align:center'>" + suf +"</td></tr></table>";
  // tbl = "<table width='12px' height='12px' style='border: 1px solid #000;background:#00ff00'><tr><td style='width:12px;align:center'>" + suf +"</td></tr></table>";
   if (suf != " ") {newTxt += "<span> " + tbl + " </span>";}
  }
  newTxt += "</span></div><div style='clear:both'></div>";
...
function process_click_global(w){
  ...
  if (i < pcd.length && j + 1 < pcd[i].length){
    ...
    if (cl == exp1 && (c2.length == 1 && c2 == sp[exp2] || (c2.length == 2 && c2 == spmf[exp2]))) {
      ...
      var newTxt = "<div><span style='line-height:16px;'>"  + tbl + "</span></div>";
